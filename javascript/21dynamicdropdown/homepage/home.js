...
function getCity(){
    ...
    .then((data) => {
        data.map((item) => {
            ...
            let text = document.createTextNode(item.state) // Hyderabad
            element.appendChild(text) //<option>Hyderabad</option>
            element.value = item.state_id //<option value="1">Hyderabad</option>
            document.getElementById('city').appendChild(element) 
            //<select><option value="1">Hyderabad</option></select>
        })
    })
}
